...
A modern GUI calculator built with CustomTkinter, featuring a dark theme inspired by ProtonVPN.
Supports basic arithmetic, history tracking, backspace, percentages, parentheses, and special functions like square, inverse, and square root.

Future enhancements planned: algebraic solvers, integrals/derivatives, number base conversions, graphing.
...
# Global variables for expression and history (can be improved with class later)
expr = "" 
...
# Load backspace icon
icon_path = os.path.join(os.path.dirname(__file__), "backspace_icon.png")
img = Image.open(icon_path)

def press(key):
    global expr 
    # Replace ^ with ** for Python eval compatibility
    if key == '^':
        key = '**'
    expr += str(key)
    display.set(expr)
...
def equal():
    ...
    try: 
        ...
        # Display result onto the calculator's history screen. 
...
# Removes most recent number entered that comes after the latest operator.
def clear_entry(): 
    global expr
    if expr: 
        i = len(expr) - 1
...
def clear_history():
    ...
    calc_history_box.delete("0.0", END)
...
def sqrt_func():
    global expr
    try:
        sqrt_result = math.sqrt(float(eval(expr)))
        past_expressions_list.append(f"√({expr})")
        past_results_list.append(sqrt_result)
        display.set(str(sqrt_result))
        calc_history_box.insert(END, f"{past_expressions_list[-1]}\n\t = {past_results_list[-1]}\n")
    except:
        display.set("error")
        expr = ""
...
if __name__ == "__main__": 
    ...
    calc_history_box = CTkTextbox(master=root, 
                         border_color="#EAE2ED", fg_color="#D7C9DB", text_color="#514354", 
                         scrollbar_button_color="#724060",
    ...
    zero_button = CTkButton(master=root, text="0", 
                            border_color="papaya whip", fg_color="#2A0C2F", bg_color="papaya whip", 
                            hover_color="#2B3A67", text_color="white",
    ...
    clear_history_button = CTkButton(master=root, text="Clear History", 
                                     border_color="papaya whip", hover_color="#2B3A67", 
                                     text_color="white", fg_color="#2A0C2F", 
    ...
    backspace_button = CTkButton(master=root, text="",
                            ...
                            command=backspace,
                            height=75, width=75, border_width=0.5, font=("Arial",28),
                            image=CTkImage(dark_image=img, light_image=img, size=(40,40)))
    ...
    inverse_button = CTkButton(master=root, text="1/x",
                               ...
                               command=inverse, 
                               height=75, width=75, border_width=0.5, font=("Arial",28))
    inverse_button.grid(row=2, column=0, padx=0, pady=0, sticky="nsew")
